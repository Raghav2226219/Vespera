...
import BoardAuditHeader from "../components/boardAudits/BoardAuditHeader";
import BoardAuditCard from "../components/boardAudits/BoardAuditCard";
import BoardAuditFilters from "../components/boardAudits/BoardAuditFilters";
...
const BoardAuditsPage = () => {
  ...
  useEffect(() => {
    ...
    const fetchLogs = async () => {
      ...
      setError("");
      try {
        const res = await api.get(`/board-audits/all?${queryString}`);
        if (!cancelled && res.data?.success) {
          setLogs(res.data.data || []);
          setTotalPages(res.data.totalPages || 1);
          setTotal(res.data.total || 0);
        } else if (!cancelled) {
          setError("Failed to fetch board audits.");
        }
      } catch (err) {
        if (!cancelled) {
          console.error("Error fetching board audits:", err);
          setError("Unable to fetch audit logs. Please try again later.");
        }
      } finally {
          ...
          {loading ? (
            ...
            <div className="relative flex-1 min-h-0 overflow-hidden">
              <div className="absolute inset-0 border-l border-yellow-400/25 pl-6 space-y-6 custom-scrollbar overflow-y-auto overflow-x-hidden pr-4">
                <AnimatePresence>
                  {logs.map((log, idx) => (
                    <BoardAuditCard key={log.id || idx} log={log} idx={idx} />
                  ))}
                </AnimatePresence>
                <div className="h-4" />
              </div>
...
export default BoardAuditsPage;
